...
const mymain = async () => {
    ...
    const getIdsFromPage = async (page: number) => {
...
/**
 * Adds the popup UI container to the document body.
 */
function addUI() {
    // popup container
...
const toggleContainerDisplay = () => {
	containerShown = !containerShown; updateContainerDisplay();
}
...
// two button to toggle ui
// one in the main ui, following steam style
...
declare const $J: any;
const loadMapListToGame = async (mapList: string[]) => {
    const url = new URL(window.location.href);
    const appid = url.searchParams.get("appid")!;
    const sessionid = g_sessionID as string;
